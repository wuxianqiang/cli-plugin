...
class DocsWebServer {
  constructor(store, options = {}) { this.store=store; this.host=options.host||'127.0.0.1'; this.port=options.port||0; this.server=null; this.clients=new Set(); }
  ...
  async handle(req,res){try{const url=new URL(req.url,`http://${req.headers.host}`);
    ...
    m=url.pathname.match(/^\/api\/workflows\/([^/]+)\/annotations$/);
    if(req.method==='GET'&&m)return this.json(res,this.store.readAnnotations(m[1]));
